...
@Component({
  ...
  styleUrls: ['./styles/addcapability.component.scss'],
...
export class AddcapabilityComponent implements OnInit {
  ...
  onSubmit(isCreateANother: boolean = false) {
    ...
    this.userRoleAuthService.saveCapability(capabilityObject).subscribe(res => {
      if (res['status'] === 200) {
        this.spinnerService.hide();
        const response: ICapability = res['data'];

        this.translate.get('msgSuccessfulRegister').subscribe((text: string) => {
          this.notificationService.showNotification('Capability created successfully', 'top', 'center', '', 'info-circle');
        });
