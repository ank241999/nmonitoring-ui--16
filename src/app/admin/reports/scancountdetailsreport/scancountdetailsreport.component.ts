...
@Component({
  selector: 'app-scancountdetailsreport',
  templateUrl: './scancountdetailsreport.component.html',
  styleUrls: ['./scancountdetailsreport.component.scss'],
  providers: [DatePipe, { provide: DateAdapter, useClass: MomentDateAdapter, deps: [MAT_DATE_LOCALE] },
    { provide: MAT_DATE_FORMATS, useValue: MAT_MOMENT_DATE_FORMATS }]
})
export class ScancountdetailsreportComponent implements OnInit {

  users: IScanCountDetails[] = [];

  displayedColumns: string[] = ['dateTime', 'totalAlarmsCount', 'gate', 'lane', 'devices', 'guardName', 'email'];
  dataSource: MatTableDataSource<IScanCountDetails>;

  @ViewChild('paginator', { static: true }) paginator: MatPaginator;
  @ViewChild('sort', { static: true }) sort: MatSort;
  selection = new SelectionModel<IScanCountDetails>(true, []);
  filtersVisible = true;
  isBrowser: boolean;
  ...
  showReport() {
    ...
    this.reportService.getScanCountDetails(this.fromDate, this.toDate).subscribe(res => {
      this.users = res;
  ...
  downloadEXCEL() {
    ...
    this.users.forEach(e => {
      const Data = {
        'DATE & TIME': (this.datePipe.transform(e.dateTime, 'MM/dd/yyyy HH:mm:ss')),
        // 'PERSON SCANNED ID': (e.objectids == null ? '' : e.objectids),
    ...
    /* save to file */
    XLSX.writeFile(wb, 'Scan Count Details Report.xlsx');
  }
  ...
  downloadPDF() {
    ...
    doc.setPage(1);
    doc.save('Scan Count Details Report.pdf');
  }
}
