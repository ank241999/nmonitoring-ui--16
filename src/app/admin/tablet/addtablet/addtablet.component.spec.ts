...
describe('AddtabletComponent', () => {
  ...
  it(`should call onScreenClose `, async () => {
    component.addtabletScreenClose();
    expect(component.addtabletScreenClose).toBeTruthy();
  });
  ...
  it('should submit the form correctly', () => {
    const mocTabletService = new MocTabletService();
    const addTabletSpy = spyOn(mocTabletService, 'addTablet').and.returnValue(of(Response));

    component.form.controls['tabletName'].setValue('Test Tablet');
    component.form.controls['tabletMacAddress'].setValue('test@example.com');
    component.form.controls['tabletStatus'].setValue(true);
    component.form.controls['primaryTablet'].setValue(false);

    component.onSubmit();
    expect(addTabletSpy).toBeTruthy();
  });

  it('should handle clipboard paste correctly', () => {
    spyOn(window, 'setTimeout').and.callFake((callback: Function) => {
      callback();
    });
    component.isPaste = false;
    component.users = [];

    expect(component.isPaste).toBe(false);
    expect(component.form.controls['tabletMacAddress'].value).toBe('');
  });
